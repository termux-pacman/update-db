...
get-object() {
	info "aws s3api get-object --bucket ${bucket} --key ${1} ${2}"
	aws s3api get-object --bucket "${bucket}" --key "${1}" "${2}"
	echo -e "\n\n"
}

put-object() {
	local cache_control_flag=""
	if ${disable_cache_policy}; then
		cache_control_flag="--cache-control 'no-cache, no-store, max-age=0, must-revalidate'"
	fi
	info "aws s3api put-object --bucket ${bucket} --key ${1} --body ${2} ${cache_control_flag}"
	if [ "${GITHUB_EVENT}" != "pull_request" ]; then
		eval "aws s3api put-object --bucket ${bucket} --key ${1} --body ${2} ${cache_control_flag}"
	fi
	echo -e "\n\n"
}

aws-rm() {
	info "aws s3 rm s3://${bucket}/${1}"
	if [ "${GITHUB_EVENT}" != "pull_request" ]; then
...
invalidation() {
	info "aws cloudfront create-invalidation --distribution-id ${DISTRIBUTION_ID} --paths /${1}"
	if [ "${GITHUB_EVENT}" != "pull_request" ]; then
...
for format in db files; do
	...
	if ! gpg --verify "${repo}.${format}.tar.gz"{.sig,}; then
...
for i in ${files_dp}; do
	if [[ "${i}" != *"${name_fdp}"*".sig" ]]; then
		...
		if gpg --verify "${name_fdp}"{.sig,}; then
			...
			warn "package removal failed, invalid signature"
		fi
		rm "${name_fdp}"*
	fi
done
...
for i in ${files_pkg}; do
	if [[ "${i}" != *".pkg."*".sig" ]]; then
		...
		if gpg --verify "${i2}"{.sig,}; then
			...
			for j in "" ".sig"; do
				put-object "${repo}/${arch}/${i2}${j}" "${i2}${j}"
				if grep -q "/${i2}${j}$" <<< "${files}"; then
					invalidation "${repo}/${arch}/${i2}${j}"
				fi
			done
			upload=true
		else
			warn "failed to update package '${i}', invalid signature"
		fi
		rm ${i2}*
	fi
done
